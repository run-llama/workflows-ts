...
import { createReport, webSearch, evaluateQueryAndEnhance } from "./openai-utils";
...
const finalResponseEvent = workflowEvent<{ output_path: string | null, refusal: string | null }>();
...
workflow.handle([userInputEvent], async (context, { data }) => {
  ...
  if (enhancedQuery.startsWith("Sorry, what you are asking is not news-related,") || enhancedQuery.startsWith("Sorry, it was not possible to process your request at this time:")) {
    sendEvent(finalResponseEvent.with({output_path: null, refusal: enhancedQuery}))
  } else {
...
workflow.handle([createReportEvent], async (context, { data }) => {
    ...
    if (reportContent.report_title) {
        const out_path = await convertMdToPdf(reportContent.report_content, reportContent.report_title)
        if (out_path.startsWith("Impossible to convert ")) {
            sendEvent(finalResponseEvent.with({output_path: null, refusal: out_path}))
        } else {
            sendEvent(finalResponseEvent.with({output_path: out_path, refusal: null}))
        }
    } else {
        sendEvent(finalResponseEvent.with({output_path: null, refusal: reportContent.report_content}))
    }
})
