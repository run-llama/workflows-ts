...
workflow.handle([incrementEvent], (context, increment) => {
  const { state } = context;
...
You can also use the signal in the context parameter to handle errors:

```ts
workflow.handle([processEvent], (context) => {
