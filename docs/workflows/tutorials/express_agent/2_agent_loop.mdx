...
In step 2, we've been missing the code to collect tool responses sent out in step 3.
We'll add that now, by updating the `userInputEvent` handler (by adding code after sending out the `toolCallEvent` events):
...
> Note that it would be helpful to store the tool responses and add a dedicated handler to collect them. We'll do that in the [next step](./3_adding_state.mdx).
...
For the complete working example, see [`demo/express/2b-agent-loop-workflow.ts`](https://github.com/run-llama/workflows-ts/blob/main/demo/express/2b-agent-loop-workflow.ts).
