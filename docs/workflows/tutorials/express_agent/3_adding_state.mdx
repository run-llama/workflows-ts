...
This section assumes you’ve completed the workflow version in the [previous step](./2_agent_loop.mdx).
...
Below are the changes needed to do these changes:
...
Simplify the initial `userInputEvent` handler a lot by storing the messages in the state and removing the complete logic of collecting tool responses:
