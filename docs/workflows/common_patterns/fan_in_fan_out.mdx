...
**4. Adding a Fan-In Handler**

In the last step of our workflow, we need to collect the results from the collecting steps and store them, so that we will be able to return them once all the events have been consumed (`completeEvent`):
