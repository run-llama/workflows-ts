...
workflow.handle([startEvent], async (event) => {
  ...
  const results = await collect(
    until(
      filter(
        stream,
        (ev) => processedValidEvent.include(ev) || processedInvalidEvent.include(ev)
      ),
      () => {
...
async function runBranchingMergingWorkflow() {
  ...
  const result = await pipeline(stream, async function* (source) {
    for await (const event of source) {
      ...
      else if (processedValidEvent.include(event) || processedInvalidEvent.include(event)) {
...
workflow.handle([initEvent], async (event) => {
  ...
  const results = await collect(
    until(
      filter(
        stream,
        (ev) => transformedEvent.include(ev)
      ),
      () => {
        numResults++;
