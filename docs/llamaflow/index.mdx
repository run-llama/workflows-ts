...
### Streaming using standard API / third-party libraries

You can use the Web Stream API to handle stream
  ...
  .pipeThrough(
    new TransformStream({
      async transform(event, controller) {
        if (toolCallResultEvent.include(event)) {
    ...
    new WritableStream({
      async write(event) {
        if (toolCallResultEvent.include(event)) {
          ...
          await openai.chat.completions.terminateToolCall({
            tool_call_id: id,
            content,
          });
        }
