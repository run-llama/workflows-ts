...
const { withStore } = createStoreMiddleware<CounterStore>(() => ({
  count: 0,
  history: [],
}));
...
async function runWithStore() {
  ...
  // The store persists across events
  console.log("Final count:", store.count);
  console.log("Final history:", store.history);
}
...
```ts
const { withStore } = createStoreMiddleware((input: { initialCount: number }) => ({
  count: input.initialCount,
  history: [input.initialCount],
}));

const workflow = withStore(createWorkflow());
const { store } = workflow.createContext({ initialCount: 10 });
```
